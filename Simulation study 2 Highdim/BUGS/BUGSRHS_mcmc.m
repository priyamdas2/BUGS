function out = BUGSRHS_mcmc(X, y, opts)
% Regularized Horseshoe (RHS) baseline MCMC
%
% Model:
%   y = X*beta + eps,  eps ~ N(0, sigma2 I)
%   beta_j | ... ~ N(0, sigma2 * kappa_j^2)
%   kappa_j^2 = [c2 * tau2 * lambda2_j] / [c2 + tau2 * lambda2_j]
%
% Priors:
...
%
% Notes:
%   - This is the regularized horseshoe baseline corresponding to the
%     BUGS model with eta fixed at 0, i.e. no guidance term.
%   - Guidance statistics zstar are not used in this baseline.
%
% INPUTS
...
for iter = 1:opts.niter
    ...
    %----------------------------------------------------------
    % Update kappa_j^2 and D for regularized horseshoe
    %----------------------------------------------------------
    numer = c2 * tau2 .* lambda2;
    kappa2 = numer ./ (c2 + tau2 .* lambda2);
    ...
    for j = 1:p
        ...
        logf = @(th) logpost_theta_j_rhs(th, beta(j), sigma2, tau2, c2);
    ...
    %----------------------------------------------------------
    phi0 = log(tau2);
    logf_phi = @(phi) logpost_phi_rhs(phi, beta, sigma2, lambda2, c2, opts.tau0);
    ...
    %----------------------------------------------------------
    psi0 = log(c2);
    logf_psi = @(psi) logpost_psi_rhs(psi, beta, sigma2, lambda2, tau2, opts.a_c, opts.b_c);
    psi_new = slice_sample_univariate(psi0, logf_psi, opts.slice_w, opts.slice_m, -Inf, Inf);
    c2 = exp(psi_new);

    %----------------------------------------------------------
    % RHS baseline: eta fixed at 0 (no guidance update)
    %----------------------------------------------------------
    eta = 0;
    ...
    if opts.verbose && (mod(iter, opts.display_every) == 0)
        fprintf('Iter %d/%d | sigma2=%.4f tau=%.4f c2=%.4f\n', ...
            iter, opts.niter, sigma2, sqrt(tau2), c2);
    end
end
...
function val = logpost_theta_j_rhs(theta, beta_j, sigma2, tau2, c2)
% theta = log(lambda_j^2) for RHS baseline
lambda2 = exp(theta);
kappa2 = (c2 * tau2 * lambda2) / (c2 + tau2 * lambda2);
...
function val = logpost_phi_rhs(phi, beta, sigma2, lambda2, c2, tau0)
% phi = log(tau^2), tau = exp(phi/2)
tau2 = exp(phi);
kappa2 = (c2 * tau2 .* lambda2(:)) ./ ...
    (c2 + tau2 .* lambda2(:));
kappa2 = max(kappa2, 1e-12);
...
function val = logpost_psi_rhs(psi, beta, sigma2, lambda2, tau2, a_c, b_c)
% psi = log(c2)
c2 = exp(psi);
kappa2 = (c2 * tau2 .* lambda2(:)) ./ ...
    (c2 + tau2 .* lambda2(:));
