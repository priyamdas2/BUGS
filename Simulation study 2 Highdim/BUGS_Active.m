...
opts.save_top_beta = 5000;        % 100 for low dims

% active-set controls
opts.active_beta_thresh = 0.001;
opts.active_cap = 10000;           % 1000 for low dims
opts.min_active = min(p,5000);    % 100 for low dims
opts.guidance_keep = min(p,1000);   % 200 for low dims
