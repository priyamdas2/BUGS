summarize_metrics_across_reps_sensitivity <- function(
    sensitivity_casenum,
    n,
    p,
    method_name,
  ...
  build_file_name <- function(rep_id) {
    sprintf(
      "Output_casenum_%d_n_%s_p_%s_%s_rep_%s.csv",
      sensitivity_casenum, n, p, method_name, rep_id
    )
  }
  ...
  if (n_rows_each == 1) {
    out_name <- sprintf(
      "Output_Summary_ALL_casenum_%d_n_%s_p_%s_%s.csv",
      sensitivity_casenum, n, p, method_name
    )
    ...
    for (r in seq_len(n_rows_each)) {
      out_name <- sprintf(
        "Output_Summary_ALL_casenum_%d_n_%s_p_%s_%s_row_%s.csv",
        sensitivity_casenum, n, p, method_name, r
      )
      out_path <- file.path(output_dir, out_name)
