make_combined_summary_table_Highdim <- function(
    scenario,
    method_names,
    n,
    p,
  ...
  build_file <- function(method) {
    file.path(
      input_dir,
      sprintf("Output_Summary_ALL_scen_%d_n_%d_p_%d_%s.csv",
              scenario, as.integer(n), as.integer(p), method)
    )
  }
  ...
  if (is.null(output_file)) {
    output_file <- sprintf("ALL_Combined_Summary_scen_%d_n_%d_p_%d.csv",
                           scenario, as.integer(n), as.integer(p))
  }
