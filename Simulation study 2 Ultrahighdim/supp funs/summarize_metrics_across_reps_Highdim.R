summarize_metrics_across_reps_Highdim <- function(
    scenario,
    n,
    p,
    method_name,
  ...
  build_file_name <- function(rep_id) {
    sprintf(
      "Output_scen_%d_n_%d_p_%d_%s_rep_%d.csv",
      scenario, as.integer(n), as.integer(p), method_name, as.integer(rep_id)
    )
  }
  ...
  if (n_rows_each == 1) {
    out_name <- sprintf(
      "Output_Summary_ALL_scen_%d_n_%d_p_%d_%s.csv",
      scenario, as.integer(n), as.integer(p), method_name
    )
    ...
    for (r in seq_len(n_rows_each)) {
      out_name <- sprintf(
        "Output_Summary_ALL_scen_%d_n_%d_p_%d_%s_%d.csv",
        scenario, as.integer(n), as.integer(p), method_name, r
      )
      out_path <- file.path(output_dir, out_name)
