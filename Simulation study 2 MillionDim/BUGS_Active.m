...
opts.display_every = 1;          % for Million dim only, default 100
...
opts.save_top_beta = 50000;        % 100 for low dims

% active-set controls
opts.active_beta_thresh = 0.001;
opts.active_cap = 100000;            % 1000 for low dims
opts.min_active = min(p,50000);      % 100 for low dims
opts.guidance_keep = min(p,50000);   % 200 for low dims
