...
# ============================================================
# Simulation study with Spike-and-Slab Lasso
# Reads data from Data/, saves metrics to Results/
# ============================================================
setwd("U:/BUGS/Simulation study")
library(SSLASSO)
...
for (rep_id in reps) {
  ...
  # SSLASSO returns a coefficient path and corresponding intercept path.
  # We select the path point with the smallest training MSE.
  fitted_mat <- sweep(X %*% fit_ssl$beta, 2, fit_ssl$intercept, FUN = "+")
  train_mse_path <- colMeans(
    (matrix(y, nrow = length(y), ncol = ncol(fitted_mat)) - fitted_mat)^2
  )
  best_idx <- which.min(train_mse_path)
  
  beta_est_ssl <- as.numeric(fit_ssl$beta[, best_idx])
  intercept_ssl <- as.numeric(fit_ssl$intercept[best_idx])
  
  metrics_ssl <- evaluate_fit(
    beta_est = beta_est_ssl,
    beta_ref = beta_true_std,
    X = X,
    comp_time = as.numeric(t_ssl["elapsed"]),
    tol = tol,
    intercept_est = intercept_ssl,
    intercept_ref = 0
  )
  
  outfile_ssl <- sprintf(
    "%s/Output_scen_%d_n_%d_p_%d_SSLASSO_rep_%d.csv",
    results_dir, scenario, n, p, rep_id
  )
  write.csv(metrics_ssl, outfile_ssl, row.names = FALSE)
