function out = BUGSRHS_Active_mcmc(X, y, opts)
% BUGS-RHS-Active: sparsity-aware approximate MCMC for
% regularized horseshoe (RHS) with active-set updates
%
% IMPORTANT:
...
if opts.verbose
    fprintf('Running %d iterations (BUGS-RHS-Active)...\n', opts.niter);
end
...
for iter = 1:opts.niter
    ...
    for kk = 1:numel(active_idx)
        ...
        logf = @(th) logpost_theta_j(th, beta(j), sigma2, tau2, c2);
    ...
    %----------------------------------------------------------
    phi0 = log(tau2);
    logf_phi = @(phi) logpost_phi(phi, beta, sigma2, lambda2, c2, opts.tau0);
    ...
    %----------------------------------------------------------
    psi0 = log(c2);
    logf_psi = @(psi) logpost_psi(psi, beta, sigma2, lambda2, tau2, opts.a_c, opts.b_c);
    ...
    if opts.verbose && (mod(iter, opts.display_every) == 0)
        fprintf('Iter %d/%d | sigma2=%.4f tau=%.4f c2=%.4f | active=%d\n', ...
            iter, opts.niter, sigma2, sqrt(tau2), c2, numel(active_idx));
    end
end
...
%--------------------------------------------------------------
% Defaults shared with BUGS_mcmc: EXACTLY MATCH where applicable
%--------------------------------------------------------------
def.niter = 6000;
def.burnin = 3000;
def.thin = 5;
...
function val = logpost_theta_j(theta, beta_j, sigma2, tau2, c2)
lambda2 = exp(theta);
...
function val = logpost_phi(phi, beta, sigma2, lambda2, c2, tau0)
tau2 = exp(phi);
...
function val = logpost_psi(psi, beta, sigma2, lambda2, tau2, a_c, b_c)
c2 = exp(psi);
kappa2 = (c2 * tau2 .* lambda2(:)) ./ ...
    (c2 + tau2 .* lambda2(:));
kappa2 = max(kappa2, 1e-12);
